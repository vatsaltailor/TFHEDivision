...
int main() {
    ...
    //generate encrypt the 16 bits of 36
    int16_t plaintext1 = 405;
    LweSample* ciphertext1 = new_gate_bootstrapping_ciphertext_array(16, params);
    for (int i=0; i<16; i++) {
        bootsSymEncrypt(&ciphertext1[i], (plaintext1>>i)&1, key);
    }

    //generate encrypt the 16 bits of 6
    int16_t plaintext2 = 17;
    LweSample* ciphertext2 = new_gate_bootstrapping_ciphertext_array(16, params);
    for (int i=0; i<16; i++) {
        bootsSymEncrypt(&ciphertext2[i], (plaintext2>>i)&1, key);
    }
    
    //generate encrypt the 16 bits of 0
    int16_t plaintext_zero = 0;
    LweSample* ciphertext_zero = new_gate_bootstrapping_ciphertext_array(16, params);
    for (int i=0; i<16; i++) {
        bootsSymEncrypt(&ciphertext_zero[i], (plaintext_zero>>i)&1, key);
    }
    
    //generate encrypt the 16 bits of 1
    int16_t plaintext_one = 1;
    LweSample* ciphertext_one = new_gate_bootstrapping_ciphertext_array(16, params);
    for (int i=0; i<16; i++) {
        bootsSymEncrypt(&ciphertext_one[i], (plaintext_one>>i)&1, key);
    }


    printf("Hi there! Today, I will ask the cloud what is the division between %d and %d\n",plaintext1, plaintext2);
    ...
    for (int i=0; i<16; i++) 
        export_gate_bootstrapping_ciphertext_toFile(cloud_data, &ciphertext_zero[i], params);
    for (int i=0; i<16; i++) 
        export_gate_bootstrapping_ciphertext_toFile(cloud_data, &ciphertext_one[i], params);
    fclose(cloud_data);
